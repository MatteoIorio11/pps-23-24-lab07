...
type Solution = Iterable[BoardPosition]
type IterableFactory = Solution => Iterable[Solution]
given IterableFactory = SortedSet(_)
...
object Solitaire extends App:
  ...
  private def solitaireLogic(board: Board)(remainingPositions: Int)(using factory: IterableFactory): Iterable[Solution] = remainingPositions match
    case 0 => factory(Set((board._1 / 2, board._2 / 2)))
    case _ => 
      for
        ...
        if isPlaceable(possiblePosition = possiblePosition, positions = positions)
      yield
        positions.toSeq :+ possiblePosition
  ...
  //placeMarks((3,3)).foreach(p => p)
  placeMarks((3,3)).toSeq.foreach(sol => println(render(sol.toSeq, 3, 3)))
